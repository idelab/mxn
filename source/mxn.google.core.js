	...
	this.Polyline = {
		...
		toProprietary: function() {
			...
			if (this.closed	|| gpoints[0].equals(gpoints[length-1])) {
				if(!gpoints[0].equals(gpoints[length-1]))	{gpoints.push(gpoints[0]);}
				gpoly = new GPolygon(gpoints, this.color, this.width || 4, this.opacity, this.fillColor || "#5462E3", this.opacity || "0.3");
			} else {
				gpoly = new GPolyline(gpoints, this.color, this.width || 4, this.opacity);
			}
			return gpoly;
		},
...
var google2D = new GoogleSettings();
mxn.register('google', google2D);

/*var google3D = new GoogleSettings();
google3D.Mapstraction.init = function(element,api) {		
	var me = this;

	if (GMap2) {
		if (GBrowserIsCompatible()) {
			this.maps[api] = new GMap2(element);
			this.maps[api].addMapType(G_SATELLITE_3D_MAP);
			this.maps[api].setMapType(G_SATELLITE_3D_MAP); 
			GEvent.addListener(this.maps[api], 'click', function(marker,location) {
...
mxn.register('google3D',google3D);*/